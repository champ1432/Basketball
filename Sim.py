...
# finds ppg by getting points value then subtracting the change
x = away_center[0][3]
y = PlayerStatsDB.get_players_points(connection, x)
c = y[0][1]
print(c)
    ...
    if pos == home_team:
        ...
        if quarter_clock > 24:
            if shot_success == 0:
                ...
                PlayerStatsDB.add_player_points(connection, shooter[0][3])
                quarter_clock = quarter_clock - random.randint(10, 24)
                minutes, seconds = divmod(quarter_clock, 60)
                print(f"{home[0][2]}: {home_score} - {away[0][2]}: {away_score}    \n{minutes}:{seconds:02d} remaining")
                pos = away_team
